...
public class mainCreateApp {
    public static void main(String[] args){
		...
		contract c2 = new contract("Panagioths", "Konstantinakos","690000001","13/02/20","card",0.45,yphresies.get(1));
		...
		cf.CreateFile(yphresies);
		CreateContractFile crconf = new CreateContractFile();
		int start =1;
		for (Services service: yphresies){
			cont= stat.returnContract(service.getId(),symvolaia);
			crconf.CreateContractFile(service, cont,start);
			start+=1;
		}//create files
        ...
        while(not) {
            //2o meros ergasias
			cf.ReadFile(yphresies);
			
			start =1;
			if (crconf.ReadContractFile(yphresies)){
				for (Services service: yphresies){
            ...
            switch(x) {
                ...
                case 2:
					...
					//adding new contracts at files
					cf.AppendFile(yphresies.get(i-1));
					cont= stat.returnContract(yphresies.get(i-1).getId(),symvolaia);
					crconf.AppendContractFile(yphresies.get(i-1), cont);
                    break;
